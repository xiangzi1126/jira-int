...
# 総評シート挿入データのフォント (MS PGothic 10pt) - UNICYCENと同一
...
# 総評 布局配置 (模板修正後の固定位置。UNICYCEN方式: 挿入行でoffset累積)
LAYOUT = {
    'greeting_row': 2,           # R2 月份挨拶
    'overview_row': 5,           # R5 1、総評 概要 (模板既定文="全体的に安定稼働しております。")
    'alert_count_row': 8,        # R8 2-1アラート件数
    'detection_row': 44,         # R44 ■検知内容 数据行 (header R43, 既定文="特にございません。")
    'incident_row': 47,          # R47 ■事象纏め 数据行 (header R46, 既定文="特にございません。")
    'resource_clear_from': 49,   # R49 3、リソース 起清空重写资源表 (2-2説明域はUNICYCEN方式で保持)
    'comments_row': 85,          # R85 【CPU】(R86)前空行, save/restore起点
}
...
def load_all_data(script_dir):
    ...
    # 结构: {host_lower: [(drive, c16_val), ...]} —— config 行顺序即人工月报的数据盘展示顺序
...
def populate_summary(wb, current_alerts, prev_alerts, cmd_data,
                     ...
                     svfs_expand_map, c11_map):
    """填充総評1-8（动态布局：插入行+R42以下清空重写）"""
    if SHEET_SUMMARY not in wb.sheetnames:
        return
    ws = wb[SHEET_SUMMARY]

    def set_cell(row, col, value):
        """セルに値を書き込み + フォント(MS PGothic 10pt) - UNICYCENと同一"""
    ...
    # === 资源表: 定位填入模板(保留合并单元格), 不再清空重建 ===
    # 模板的 3、リソース/3-1/表头/合并/課金/sections 都保留, 只往主机名行填数据
...
def _fill_resource_table_inplace(ws, cmd_data, host_mapping):
    ...
    保留模板合并(B:C主机名, M:N/P:Q使用率, A列環境)。不再清空重建。"""
    ...
    for display_name in order:
        ...
        ws.cell(host_row, 13).value = system_row.get('磁盘最大使用率（%)', '')           # M (M:N merged)
        if data_row is not None:
            ws.cell(host_row, 15).value = _ceil_int(data_row.get('磁盘大小(GB)', ''))  # O 数据盘大小
            ws.cell(host_row, 16).value = data_row.get('磁盘最大使用率（%)', '')         # P (P:Q merged)
        else:
            ws.cell(host_row, 15).value = '-'
            ws.cell(host_row, 16).value = '-'
        filled += 1
    logger.info(f"【総評】第一表(定位填入)完成, {filled}台主机")


def _fill_svfs_table_inplace(ws, cmd_data, svfs_expand_map):
    """第二资源表(SVFS): 定位模板主机名, 主机级 D-M 填首行(跨行合并的top-left),
    数据盘 N/O/P 每盘一行。保留模板合并。不再清空重建。"""
    cpu_row = _find_cpu_row(ws) or (ws.max_row + 1)
    svfs_hosts = {}
    for _, row in cmd_data.iterrows():
        instance_name = str(row.get('InstanceName', ''))
...
def _replace_section_months(ws, target_year, target_month):
    """section 7 の M月/N月 占位符置換 (7-1 M月の作業状況 -> 6月, 7-2 N月の作業予定 -> 7月)"""
...
def _create_alert_chart(wb, summary_ws, anchor_cell):
    ...
    # 绘画区上方留白 10%, 底部留 5%(与リソース图一致; y=0.10, h=0.85)
    ...
    chart.layout = Layout(
        manualLayout=ManualLayout(
            ...
            x=0.08, y=0.10, w=0.90, h=0.85,
        )
    )

    summary_ws.add_chart(chart, anchor_cell)
    # 强制设置图表尺寸（33cm宽 x 14cm高）
    _set_chart_size(summary_ws._charts[-1], anchor_cell, 33, 14)
...
def populate_trend_sheet(wb, host_list, current_alerts, prev_alerts):
    ...
    # 先取消数据区域的合并单元格
    merged_to_remove = [mr for mr in list(ws.merged_cells.ranges)
                        if mr.max_row >= TREND_DATA_START_ROW]
    for mr in merged_to_remove:
        ws.unmerge_cells(str(mr))
...
def populate_resource_sheet(wb, sheet_name, metric_type, host_list, data_dir,
    ...
    用 ScatterChart(数值X轴) 画折线: 横轴首尾固定(上月1号~月末)5等分(m月d日, 无横轴标题, 关竖向网格线),
    纵轴百分比(min=数据最小值向下取整到10的倍数 3%->0/33%->30%, max=数据最大值向上取整到5的倍数, 无"使用率"标题),
    绘画区上方留大边距(ManualLayout y=0.25)。
    ScatterChart 数值轴比 openpyxl DateAxis 在 Excel 渲染更可靠(DateAxis 易整图不显示)。
    参考 make_monthly_opreport.py(不清列, 图表数据源保留)。"""
    ...
    # 横轴范围: 上个月(=目标月)1号~月末, 首尾固定后5等分(见下方 majorUnit)
    ...
    for host_idx, host in enumerate(host_list):
        # 每主机独立2列(datetime+average), 避免数据点(359)远超行间隔(18)导致重叠覆盖
        ...
        if not monitor_df.empty and 'datetime' in monitor_df.columns:
            ...
            # 写真实 datetime(数值X轴读为日期序列号) + average 数值；不再清列(清列会让图表数据源失效)
            ...
            chart.width = 22
            chart.height = 8
            # 纵轴: 百分比刻度; min=数据最小值向下取整到10的倍数(3%->0, 33%->30%),
            #       max=数据最大值向上取整到5的倍数; 不显示"使用率"标题
            ...
            if y_max <= y_min:  # 防御: 数据平坦且落在5倍数时保证 max>min
                y_max = y_min + 5
            ...
            chart.y_axis.tickLblPos = 'nextTo'
            chart.y_axis.delete = False
            # 横轴: 数值轴(日期序列号), 首尾固定(上月1号~月末)+固定5等分, 无轴标题
            # majorUnit=区间/5; 微膨胀 unit 防 float 下溢致末刻度落回月末前1秒(误显为倒数第2天)
            N_SEGMENTS = 5  # 5等分->6刻度(6月: 6/1 6/6 6/12 6/18 6/24 6/30)
            x_unit = (x_max - x_min) / N_SEGMENTS * (1 + 1e-9)
            chart.x_axis.number_format = 'm月d日'
            chart.x_axis.scaling.min = x_min
            chart.x_axis.scaling.max = x_min + N_SEGMENTS * x_unit  # 末刻度恰落月末当天
            chart.x_axis.majorUnit = x_unit
            chart.x_axis.majorGridlines = None  # 关闭竖向网格线
            chart.x_axis.tickLblPos = 'low'
            chart.x_axis.delete = False
            chart.legend = None  # 单系列不需要图例
            # 绘画区上方留白: ManualLayout y=0.10(标题与折线之间留白 10%)
            # 注意: 设 chart.layout 而非 chart.plot_area.layout —— openpyxl _write() 会用
            #       self.layout 覆盖 plot_area.layout, 直接设 plot_area.layout 不生效(被清空)
            chart.layout = Layout(
                manualLayout=ManualLayout(
                    layoutTarget='inner',
                    xMode='edge', yMode='edge',
            ...
            series.marker = Marker(symbol='none')  # 不显示散点, 只画线
            series.graphicalProperties = GraphicalProperties()
            series.graphicalProperties.line = LineProperties(
                solidFill="4472C4",  # 蓝色
                w=12700  # 1pt = 12700 EMU
            )
...
def add_above_marker(wb):
    """在最后一个 B 列含 ■ 的 sheet(=Disk, 最后一个リソースsheet) 的 M 列添加 "以上"。
    以该 sheet B 列最后一个含 ■ 的格为锚点, 锚点行 + RESOURCE_INTERVAL_ROWS 的 M 列写 "以上"。
    例: 锚点 ■SVVS01(Disk使用率) 在 B291 -> M309 写 "以上"。
    从最后一个 tab 倒序找含 ■ 的 sheet, 避开无 ■ 的 推移/表紙。"""
    target_ws = None
    anchor_row = None
    for ws in reversed(wb.worksheets):
        last_bullet = None
        for cell in ws['B']:  # B 列全部单元格
...
def build_project_report(data, target_year, target_month, today):
    ...
    if not cmd_proj.empty:
        ...
        # 保留每个主机的所有盘符行（系统盘/数据盘分别填列），仅按(主机,盘符)去重
        cmd_proj = cmd_proj.drop_duplicates(subset=['InstanceName', '盘符'], keep='first')
    ...
    # 最后一个リソースsheet(Disk) 末尾 M 列添加 "以上"(锚点=该 sheet B 列最后含 ■ 的格)
    add_above_marker(wb)
    ...
    _fix_chart_sizes_in_xlsx(output_path, {
        1: (33, 14),   # 総評 BarChart: 33cm x 14cm
