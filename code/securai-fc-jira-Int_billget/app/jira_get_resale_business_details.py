...
def get_jira_issue_data():
    ...
    try:
        ...
        jql = f"({' OR '.join(project_conditions)}) AND \"request type\" IN (\"業務 :課金代行\", \"业务 :课金代行\", \"Service : Billing Agency\") AND \"平台[select list (multiple choices)]\" = \"阿里云国际站\" "
        # 包含所有需要的字段（新增税率字段）
        ...
        data_file = os.path.join(data_dir, 'resale_business_details.csv')
