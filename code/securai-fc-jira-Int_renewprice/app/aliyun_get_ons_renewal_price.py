# -*- coding: utf-8 -*-
"""
阿里云 ONS (Message Queue for Apache RocketMQ) 专属续费询价工具 (账单溯源版)
功能：
1. 【账单溯源】：放弃 API 探针，通过实例 ID 自动反查历史真实消费账单（上月或去年同月）获取最终价。
2. 【动态过滤】：筛选产品代码包含 'ons' 的云资源，精准定位 ONS (Message Queue) 相关产品。
...
# 日志文件已修改为 ons 专属
LOG_FILE = os.path.normpath(os.path.join(BASE_DIR, '../../jira/log/ons_renewal_price.log'))
...
class AliyunOnsRenewalPriceQuery:
    ...
    def main():
        logger.info("=" * 100)
        logger.info("  🚀 阿里云 ONS (Message Queue for Apache RocketMQ) 续费估价工具 (账单溯源版)")
        logger.info("=" * 100)
        ...
        logger.info(f"📅 全局筛选条件: 仅处理到期时间在 [{next_month_str}] 期间的 ONS 资源")
        ...
        duration_map = AliyunOnsRenewalPriceQuery.load_duration_mapping()
        ...
        with open(input_csv_path, 'r', encoding='utf-8-sig') as f:
            for row in csv.DictReader(f):
                ...
                # 专门过滤 ONS 产品 + 严格筛选下月到期
                if pcode == 'ons' and acc:
        ...
        logger.info(
            f"📊 基础过滤完成：CSV 共扫描 {total_count} 行，筛选出 {filtered_count} 个符合【ONS + 下月到期】条件的实例。")
        ...
        if not instances_by_account:
            logger.warning(f"⚠️ 没有在清单中发现 ONS 资源，程序正常退出。")
            return

        for account, items in instances_by_account.items():
            logger.info(f"\n🏁 账号集群: [{account}] | 符合条件的 ONS 实例总数: {len(items)}")
            ...
            for item in items:
                ...
                logger.info(f"\n" + "🔍 " * 25)
                logger.info(f"   [ONS Details] 正在溯源 Message Queue 明细:")
                logger.info(f"      ├─ 📌 资源 ID      : {instance_id}")
                logger.info(f"      ├─ ⏳ 目标续费时长 : {duration_months} 个月")
                logger.info(f"      └─ 📅 当前到期时间 : {expire_time}")

                target_bill_month = AliyunOnsRenewalPriceQuery.calculate_target_bill_month(
                ...
                if target_bill_month:
                    ...
                    trade, original, curr, desc = AliyunOnsRenewalPriceQuery.query_price_from_history_bill(
...
if __name__ == '__main__':
    AliyunOnsRenewalPriceQuery.main()
