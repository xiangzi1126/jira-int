...
def init_logger():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(log_dir, 'ecs_disk_metrics.log'), encoding='utf-8'),
...
def query_disk_metrics(cms_client: CmsClient, instance_id: str, start_time: str, end_time: str) -> List[Dict]:
    """查询单个实例的 Disk (磁盘) 利用率，自动翻页"""
    all_datapoints = []
    cursor = None
    ...
    for _ in range(200):
        req = cms_models.DescribeMetricListRequest(
            ...
            # 指标改为 diskusage_utilization (ECS 磁盘使用率)
            metric_name='diskusage_utilization',
            period='7200',
            start_time=start_time,
            end_time=end_time,
            # 注意：这里我们不传入具体的 device 参数，这样阿里云会自动返回该实例下所有磁盘的数据
...
def write_instance_csv(account_dir: str, instance_id: str, instance_name: str, billing_cycle: str,
    ...
    """每个实例写一个CSV文件，文件名使用 disk_<资源名称>.csv"""
    # 【核心修改点】新增了 "盘符" 这一列
    fields = ['账期', '资源所属账号', '资源id', '资源名称', '盘符', 'timestamp', 'average', 'maximum', 'minimum']
    safe_name = re.sub(r'[\\/*?:"<>|]', '_', instance_name)

    # 文件名前缀改为 disk_
    out = os.path.join(account_dir, f'disk_{safe_name}.csv')
    ...
    with open(out, 'w', encoding='utf-8-sig', newline='') as f:
        ...
        for dp in datapoints:
            writer.writerow({
                ...
                '盘符': dp.get('device', dp.get('diskname', '')),
...
def main():
    logger.info("=" * 40)
    logger.info("🚀 开始执行 ECS [磁盘(Disk)] 监控数据拉取任务...")
    ...
    for role_section in role_sections:
        ...
        try:
            ...
            for instance_id, instance_name in instance_info.items():
                logger.info(f"  📊 正在拉取[磁盘]实例: {instance_name} ({instance_id})")
                datapoints = query_disk_metrics(cms_client, instance_id, start_time, end_time)

                if not datapoints:
                    logger.warning(f"    [警告] 实例 {instance_name} 没有查询到任何磁盘监控数据(可能未安装云监控插件)。")
                    continue
