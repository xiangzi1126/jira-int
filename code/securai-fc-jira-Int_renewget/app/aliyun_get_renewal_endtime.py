...
class AliyunMultiRoleRenewalQuery:
    ...
    @staticmethod
    def _get_jira_config_path() -> str:
        """获取Jira配置文件路径（用于读取过滤规则）"""
        return os.path.normpath(os.path.join(os.path.dirname(__file__), '../../jira/config/jira_config.ini'))

    @staticmethod
    def load_filters() -> tuple[set, set]:
        """
        读取 jira_config.ini 中的 getrenew_filter 配置，
        返回需要排除的 账号(exclude_accounts) 和 产品类型(exclude_types) 集合
        """
        config_path = AliyunMultiRoleRenewalQuery._get_jira_config_path()
        exclude_accounts = set()
        exclude_types = set()

        if not os.path.exists(config_path):
            logger.warning(f"Jira配置文件不存在，跳过过滤逻辑：{config_path}")
            return exclude_accounts, exclude_types

        config = configparser.ConfigParser()
        config.read(config_path, encoding='utf-8')

        if config.has_section('getrenew_filter'):
            # 解析排除的账号
            if config.has_option('getrenew_filter', 'exclude_accounts'):
                accounts_str = config.get('getrenew_filter', 'exclude_accounts')
                if accounts_str:
                    exclude_accounts = set([acc.strip() for acc in accounts_str.split(',') if acc.strip()])

            # 解析排除的产品类型
            if config.has_option('getrenew_filter', 'exclude_types'):
                types_str = config.get('getrenew_filter', 'exclude_types')
                if types_str:
                    exclude_types = set([t.strip() for t in types_str.split(',') if t.strip()])

            logger.info(f"✅ 读取过滤配置成功：过滤账号={exclude_accounts}，过滤产品类型={exclude_types}")
        else:
            logger.info("未找到 [getrenew_filter] 配置块，跳过过滤逻辑")
    ...
    @staticmethod
    def query_single_role(role_section: str, exclude_accounts: set, exclude_types: set) -> List[Dict[str, Any]]:
        """查询单个角色的续费实例数据"""
        logger.info(f"\n" + "-" * 50)
        ...
        try:
            ...
            # 【优化】如果当前账号在黑名单中，直接提前跳过查询，节约接口调用开销
            if original_session_name in exclude_accounts:
                logger.info(
                    f"⏭️ 角色{role_section} 的所属账号 [{original_session_name}] 在 exclude_accounts 黑名单中，已跳过查询")
                return []
    ...
    def main(args: List[str]) -> None:
        ...
        try:
            # 1. 获取过滤配置 (新增)
            exclude_accounts, exclude_types = AliyunMultiRoleRenewalQuery.load_filters()
            ...
            for role in role_sections:
                instances = AliyunMultiRoleRenewalQuery.query_single_role(role, exclude_accounts, exclude_types)
                if instances:
