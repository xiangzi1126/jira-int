# -*- coding: utf-8 -*-
"""
阿里云 SmartAG (智能接入网关) 专属续费询价工具 (账单溯源版)
功能：
1. 【账单溯源】：放弃 API 探针，通过实例 ID 自动反查历史真实消费账单（上月或去年同月）获取最终价。
2. 【动态过滤】：筛选产品代码包含 'smartag' 的云资源，精准定位 Smart Access Gateway 相关产品。
...
# 日志文件已修改为 smartag 专属
LOG_FILE = os.path.normpath(os.path.join(BASE_DIR, '../../jira/log/smartag_renewal_price.log'))
...
class AliyunSmartagRenewalPriceQuery:
    ...
    def main():
        logger.info("=" * 100)
        logger.info("  🚀 阿里云 SmartAG (智能接入网关) 续费估价工具 (账单溯源版)")
        logger.info("=" * 100)
        ...
        logger.info(f"📅 全局筛选条件: 仅处理到期时间在 [{next_month_str}] 期间的 SmartAG 资源")
        ...
        duration_map = AliyunSmartagRenewalPriceQuery.load_duration_mapping()
        ...
        with open(input_csv_path, 'r', encoding='utf-8-sig') as f:
            for row in csv.DictReader(f):
                ...
                # 专门过滤 SmartAG 产品 + 严格筛选下月到期
                if pcode == 'smartag' and acc:
        ...
        logger.info(
            f"📊 基础过滤完成：CSV 共扫描 {total_count} 行，筛选出 {filtered_count} 个符合【SmartAG + 下月到期】条件的实例。")
        ...
        if not instances_by_account:
            logger.warning(f"⚠️ 没有在清单中发现 SmartAG 资源，程序正常退出。")
            return

        for account, items in instances_by_account.items():
            logger.info(f"\n🏁 账号集群: [{account}] | 符合条件的 SmartAG 实例总数: {len(items)}")
            ...
            for item in items:
                ...
                logger.info(f"\n" + "🔍 " * 25)
                logger.info(f"   [SmartAG Details] 正在溯源 智能接入网关 明细:")
                logger.info(f"      ├─ 📌 资源 ID      : {instance_id}")
                logger.info(f"      ├─ ⏳ 目标续费时长 : {duration_months} 个月")
                logger.info(f"      └─ 📅 当前到期时间 : {expire_time}")

                target_bill_month = AliyunSmartagRenewalPriceQuery.calculate_target_bill_month(
                    expire_time, duration_months)
                ...
                if target_bill_month:
                    ...
                    trade, original, curr, desc = AliyunSmartagRenewalPriceQuery.query_price_from_history_bill(
                        instance_id, target_bill_month)

                out_row = {
                    '资源id': instance_id,
                    '资源所属账号': account,
                    '资源到期时间': expire_time,
                    '产品代码': item.get('产品代码', 'smartag'),
                    '描述': '', '原价': '', '折扣': '',
                    '货币单位': '', '最终价': ''
                }
...
if __name__ == '__main__':
    AliyunSmartagRenewalPriceQuery.main()
