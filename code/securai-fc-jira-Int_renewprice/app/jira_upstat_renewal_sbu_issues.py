...
def batch_update_jira_status():
    ...
    try:
        ...
        target_issues = []
        # 统计本月到期但续费方式非"手动续费"而被跳过的数量
        skipped_by_renew_method = 0
        # 获取当前年月（用于判断是否为本月）
        ...
        with open(jira_issue_path, 'r', encoding='utf-8') as f:
            ...
            # 校验必要列
            required_fields = ['Issue Key', '到期时间', '续费方式']
            if not all(field in reader.fieldnames for field in required_fields):
                logger.error("jira_get_renewal_sbu_issues.csv 缺少必要列（Issue Key、到期时间 或 续费方式）")
                return

            for row in reader:
                issue_key = row['Issue Key'].strip()
                endtime_str = row['到期时间'].strip()
                renew_method = row['续费方式'].strip()
                ...
                if endtime_year == current_year and endtime_mon == current_month:
                    # 筛选：仅处理续费方式为"手动续费"的Issue
                    if renew_method != '手动续费':
                        skipped_by_renew_method += 1
                        logger.debug(f"Issue {issue_key} 本月到期但续费方式为[{renew_method or '空'}]，非手动续费，跳过")
                        continue
                    ...
                    logger.debug(f"Issue {issue_key} 到期时间{endtime_str}为本月且手动续费，加入待处理列表")

        if skipped_by_renew_method:
            logger.info(f"共 {skipped_by_renew_method} 个本月到期Issue因续费方式非手动续费被跳过")
        if not target_issues:
            logger.info(f"未找到到期时间为{current_year}年{current_month}月且续费方式为手动续费的Jira工作项，无需执行状态修改")
            return
        logger.info(f"共筛选出 {len(target_issues)} 个到期时间为本月且手动续费的Jira工作项")
