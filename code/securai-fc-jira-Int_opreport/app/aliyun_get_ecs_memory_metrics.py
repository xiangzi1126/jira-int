...
def init_logger():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(log_dir, 'ecs_memory_metrics.log'), encoding='utf-8'),
...
def query_memory_metrics(cms_client: CmsClient, instance_id: str, start_time: str, end_time: str) -> List[Dict]:
    """查询单个实例的Memory(内存)利用率，自动翻页"""
    all_datapoints = []
    cursor = None
    ...
    for _ in range(200):
        req = cms_models.DescribeMetricListRequest(
            ...
            # 指标改为 memory_usedutilization (ECS 内存使用率)
...
def main():
    logger.info("=" * 40)
    logger.info("🚀 开始执行 ECS [内存(Memory)] 监控数据拉取任务...")
    ...
    for role_section in role_sections:
        ...
        try:
            ...
            for instance_id, instance_name in instance_info.items():
                logger.info(f"  📊 正在拉取[内存]实例: {instance_name} ({instance_id})")
                datapoints = query_memory_metrics(cms_client, instance_id, start_time, end_time)

                if not datapoints:
                    logger.warning(f"    [警告] 实例 {instance_name} 没有查询到任何内存监控数据(可能未安装云监控插件)。")
                    continue
