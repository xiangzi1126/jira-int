"""获取 RocketMQ 监控数据 - ConsumerLagPerGid (消费堆积)"""
...
def init_logger():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(log_dir, 'mq_metrics.log'), encoding='utf-8'),
...
NAMESPACE = 'acs_ons'
METRIC_NAME = 'ConsumerLagPerGid'  # 消费堆积
...
def get_mq_instances(role_session_name: str) -> List[Dict]:
    """从CSV获取RocketMQ实例列表"""
    mq_file = os.path.join(DATA_DIR, 'aliyun_mq_op.csv')
    instances = []
    if not os.path.exists(mq_file):
        logger.warning(f"RocketMQ盘点文件不存在: {mq_file}")
        return instances
    with open(mq_file, encoding='utf-8-sig') as f:
        for row in csv.DictReader(f):
            if row.get('资源所属账号') == role_session_name:
                instances.append({
                    'instance_id': row.get('InstanceId', '').strip(),
                    'instance_name': row.get('InstanceName', row.get('InstanceId', '')).strip(),
                    'group_id': row.get('GroupId', '').strip(),
                })
    return instances


def query_metrics(cms_client: CmsClient, instance_id: str, group_id: str,
    ...
    for _ in range(200):
        ...
        body = resp.body
        if body.code != '200':
            logger.error(f"MQ {instance_id} 查询失败: {body.message}")
            break
...
def write_csv(account_dir: str, instance_name: str,
    ...
    out = os.path.join(account_dir, f'mq_{safe_name}.csv')
...
def main():
    logger.info("=" * 40)
    logger.info("开始执行 RocketMQ 监控数据拉取任务...")
    ...
    for role_section in role_sections:
        try:
            ...
            instances = get_mq_instances(role_session_name)
            if not instances:
            ...
            for inst in instances:
                ...
                datapoints = query_metrics(
                    cms_client, inst['instance_id'], inst.get('group_id', ''),
                    start_time, end_time
                )
                if not datapoints:
                    logger.warning(f"    {inst['instance_name']} 无数据")
                    continue
                out = write_csv(account_dir, inst['instance_name'],
    ...
    logger.info(f"RocketMQ监控拉取完成，共 {total} 条数据")
