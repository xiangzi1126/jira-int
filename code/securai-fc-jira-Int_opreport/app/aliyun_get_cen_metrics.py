...
def init_logger():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(log_dir, 'cen_metrics.log'), encoding='utf-8'),
...
NAMESPACE = 'acs_cen'
METRIC_NAME = 'InternetOutBps'  # CEN发送带宽
...
def get_cen_instances(role_session_name: str) -> List[Dict]:
    """从CSV获取CEN实例列表"""
    cen_file = os.path.join(DATA_DIR, 'aliyun_cen_op.csv')
    instances = []
    if not os.path.exists(cen_file):
        logger.warning(f"CEN盘点文件不存在: {cen_file}")
        return instances
    with open(cen_file, encoding='utf-8-sig') as f:
        for row in csv.DictReader(f):
            if row.get('资源所属账号') == role_session_name:
                instances.append({
                    'instance_id': row.get('CenId', '').strip(),
                    'instance_name': row.get('Name', row.get('CenId', '')).strip(),
                })
    return instances


def query_metrics(cms_client: CmsClient, instance_id: str,
    ...
    for _ in range(200):
        ...
        body = resp.body
        if body.code != '200':
            logger.error(f"CEN {instance_id} 查询失败: {body.message}")
            break
...
def write_csv(account_dir: str, instance_name: str,
    ...
    out = os.path.join(account_dir, f'cen_{safe_name}.csv')
...
def main():
    ...
    for role_section in role_sections:
        try:
            ...
            instances = get_cen_instances(role_session_name)
            if not instances:
            ...
            for inst in instances:
                ...
                datapoints = query_metrics(cms_client, inst['instance_id'], start_time, end_time)
                if not datapoints:
    ...
    logger.info(f"CEN监控拉取完成，共 {total} 条数据")
